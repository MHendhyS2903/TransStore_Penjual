...
class HomeFragmentViewModel : BaseObservableViewModel<MenuNavigator>() {

    private val itemsRecomend = ObservableArrayList<DataItemValue>()

    fun getItemList(account: ItemRequest) {
        navigator.showLoading()
        var params = account
        var jsonString = ObjectMapper().writeValueAsString(params)
        Log.d("ParamsLogin", jsonString)
        Rx2AndroidNetworking.get(getItems)
            ...
            .subscribe(object : SingleObserver<String> {
                override fun onSuccess(response: String) {
                    ...
                    var msg = mto(response, MsgItem::class.java)
                    if(msg.status == true) {
                        setItems(msg.value as MutableList<DataItemValue>)
                        navigator.onSuccessDisplay(msg)
                    }
                }
    ...
    fun setItems(items: MutableList<DataItemValue>) {
        itemsRecomend.clear()
        itemsRecomend.addAll(items)
    }

    @Bindable
    fun getItems(): ObservableArrayList<DataItemValue> = itemsRecomend
}