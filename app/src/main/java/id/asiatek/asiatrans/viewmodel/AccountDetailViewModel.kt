...
class AccountDetailViewModel : BaseObservableViewModel<AccountNavigator>() {
        ...
        Rx2AndroidNetworking.post(PostLoginHP)
            ...
            .subscribe(object : SingleObserver<String> {
                override fun onSuccess(response: String) {
                    ...
                    Log.d(
                        LoginActivity.TAG,
                        "onResponse isMainThread : ${Looper.myLooper() == Looper.getMainLooper()}"
                    )
                    Log.d(LoginActivity.TAG, "RESPONSE : $response")
                    var msg = mto(response, MsgGmail::class.java)
                    navigator.onSuccess(msg)
                }
